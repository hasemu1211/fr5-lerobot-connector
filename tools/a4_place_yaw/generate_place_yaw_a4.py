...
try:
    from tools.a4_place_yaw.region_layout import (
        ...
        PRINT_Y_MARGIN_MM, REGION_LAYOUT_REVISION, WORKSPACE_REGIONS,
        make_red_blue_region_layout, validate_region_layout, workspace_region,
    )
except ModuleNotFoundError:
    try:  # Imported while tools/fr5_data_factory.py is the entry point.
        from a4_place_yaw.region_layout import (
            PAGE_H_MM, PAGE_W_MM, PLACE0_XY_MM, PRINT_X_MARGIN_MM,
            PRINT_Y_MARGIN_MM, REGION_LAYOUT_REVISION, WORKSPACE_REGIONS,
            make_red_blue_region_layout, validate_region_layout,
            workspace_region,
        )
    except ModuleNotFoundError:  # Direct documented generator entry point.
        from region_layout import (
            PAGE_H_MM, PAGE_W_MM, PLACE0_XY_MM, PRINT_X_MARGIN_MM,
            PRINT_Y_MARGIN_MM, REGION_LAYOUT_REVISION, WORKSPACE_REGIONS,
            make_red_blue_region_layout, validate_region_layout,
            workspace_region,
...
def make_region_svg(
    layout: dict, place_id: str,
    measured_scale_mm: float = NOMINAL_SCALE_BAR_MM,
) -> str:
    layout = validate_region_layout(layout)
    region = workspace_region(layout, place_id)
    calibration = print_calibration(measured_scale_mm)
    content_scale = calibration["content_scale_percent"] / 100.0
    lines = [
        '<?xml version="1.0" encoding="UTF-8"?>',
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{PAGE_W_MM}mm" height="{PAGE_H_MM}mm" viewBox="0 0 {PAGE_W_MM} {PAGE_H_MM}" data-place-id="{place_id}" data-region-id="{region["region_id"]}" data-layout-digest="{layout["layout_digest"]}" data-measured-scale-mm="{fmt(measured_scale_mm)}" data-content-scale-percent="{fmt(calibration["content_scale_percent"])}">',
        '<rect width="297" height="210" fill="white"/>',
        f'<g transform="translate({fmt(PAGE_W_MM / 2)} {fmt(PAGE_H_MM / 2)}) scale({content_scale:.9f}) translate(-{fmt(PAGE_W_MM / 2)} -{fmt(PAGE_H_MM / 2)})">',
    ]
    sheet = [
    ...
    lines.extend([
        ...
        svg_text(
            center_x, center_y + 4, region["display_name"], 18,
            fill=region["color"], font_weight="700", text_anchor="middle",
        ),
        svg_text(
            center_x, center_y + 14, place_id, 5,
            fill=region["color"], font_weight="700", text_anchor="middle",
        ),
    ])
    lines.extend(["</g>", "</svg>"])
...
def self_check() -> None:
    ...
    assert 'data-region-id="RED"' in make_region_svg(region_layout, "PLACE_A", 96)
    assert 'data-region-id="BLUE"' in make_region_svg(region_layout, "PLACE_B", 96)
    assert 'data-content-scale-percent="104.167"' in make_region_svg(
        region_layout, "PLACE_A", 96,
    )
...
def main() -> None:
    ...
    if args.red_blue_zone:
        ...
        zone_json = zone_dir / f"a4_place_a_red_place_b_blue_{REGION_LAYOUT_REVISION}{suffix}.json"
        zone_json.write_text(
            json.dumps(layout, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        print(zone_json)
        for place_id, region_id, _color in WORKSPACE_REGIONS:
            stem = f"a4_{place_id.lower()}_{region_id.lower()}_{REGION_LAYOUT_REVISION}{suffix}"
            zone_svg = zone_dir / f"{stem}.svg"
            zone_svg.write_text(
                make_region_svg(layout, place_id, measured_scale_mm),
                encoding="utf-8",
            )
            print(zone_svg)
            if args.pdf:
