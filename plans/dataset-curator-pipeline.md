...
- 상태: **current-main Curator software 통합 완료 — GO**. `origin/main` `3a8383582bcb90277ed1c917c62e475a6358c6a0`에서 기존 v1.2 candidate lifecycle을 그대로 재사용하고, 별도 offline `setup export/preview/finalize` surface만 의미 단위로 통합했다. Integration commit `ff22b9a2fa97c1bf4ded66258bbd75b2e2cacc9f`와 retry-safety commit `038d9f04468713dd9fa5c075b7c73b3766479124`가 존재하며, strict-warning Curator focused test는 **80/80 PASS**, final tree repository 전체 suite는 **726/726 PASS**다.
...
1. `setup export --source <frozen-source>`가 current-main canonical collection profile v2를 immutable request에 결속하고 reference PNG를 external asset root에 만든 뒤 LabelMe가 JSON을 저장해야 할 exact future path를 보고한다. setup owner가 그 PNG를 LabelMe로 열어 보고된 `reference.json`에 `TABLE_WORK_SURFACE`, A/B page corner 여덟 점, `visual_motion_support`와 필요한 grounding context polygon을 저장한 뒤 `setup preview`로 review-only overlay·processed reference·H.264를 확인한다. 기존 request는 export 당시 canonical profile path/digest를 계속 사용하며 새 default로 암묵 이행하지 않는다. `setup finalize`는 같은 preview와 producer-owned `VERIFIED` binding이 정확히 일치할 때만 canonical profile을 쓴다. 어느 setup artifact도 candidate·training authority를 만들지 않는다.
...
Integration commit `ff22b9a2fa97c1bf4ded66258bbd75b2e2cacc9f`와 retry-safety commit `038d9f04468713dd9fa5c075b7c73b3766479124`에 기록된 current-main remediation 결과는 Curator **80/80 PASS**(r002/r003 공존과 export/preview failure→exact retry 포함), episode ledger/software/training-authority contract **31/31 PASS**, live collection-profile와 one-candidate/one-ledger 선택 검사 **2/2 PASS**, final tree repository 전체 **726/726 PASS**다. Export/preview는 current invocation이 exclusive-create한 exact directory identity만 실패 시 제거하고 pre-existing/superseded valid evidence는 보존한다. Ruff 0.12.11 format/check, `compileall`과 `git diff --check`도 통과했다. Full suite에 필요한 ignored A4 fixture, `.venv` symlink, pinned submodules와 checked-in vendor patch는 transient하게 provision한 뒤 제거·reverse했으며 tracked source와 실제 dataset/output은 바꾸지 않았다. Documentation governance `audit/check`의 기존 결과는 이번에 바꾸지 않은 `plans/collection-operator-architecture-refactor.md`의 missing `closed-loop-rollout-observatory.md` link 한 건으로 exit 1이고, 이 branch의 local-only `.mex/` 검사도 scaffold 없음으로 exit 1이므로 historical PASS를 재표기하지 않는다.
...
- 새 export는 fresh `fr5-up-wrist-fixed-view-r003` namespace와 collection v2를 기본으로 하고 immutable request가 pin한 기존 canonical v1과 r002 asset도 digest 그대로 재현해 r002 geometry evidence를 다시 묻거나 재작성하지 않는다. r003은 geometry 변경이나 `VERIFIED` 승격이 아니다.
- export/preview 실패는 current invocation이 만든 exact directory만 정리해 같은 ID로 재시도할 수 있고, 이미 완성된 preview/revision은 superseded된 뒤에도 보존한다.
