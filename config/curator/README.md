...
This curator creates a separate LeRobot v3 dataset with a deterministic fixed up-camera view and an unmodified wrist-camera input path. It never edits a producer dataset, grants training authority, changes a region binding, or becomes a prerequisite for the existing raw-dataset training path.
...
Derivation uses the official LeRobot reader/writer in a run-owned sibling temporary root and pins H.264/ultrafast/CRF 23. The pinned LeRobot 0.6.1 local reader is fenced against both metadata and data/video Hub fallback; incomplete local files fail without a network probe or source write. Approved mask and plate bytes are opened with no symlink following, checked against both profile and manifest, decoded once, retained in memory, and the approval/bundle is revalidated immediately before publication. Only up calls `apply_up_view(raw_up, mask, plate)`; wrist is passed directly to the writer and is absent from that API. State, action, one exact task per episode, timestamp/index order, full video decode, actual ffprobe H.264 identity, codec-bound up/wrist pixels, and the existing dataset validator must pass before Linux `renameat2(RENAME_NOREPLACE)` publication. Per-frame source provenance is copied byte-for-byte; source timing-quality fields are preserved with explicit lineage while all derived up/wrist image metrics are recomputed before validator replay. Training approval is never copied, and a quarantined source is rejected. On failure, writer finalization precedes cleanup; cleanup deletes only the captured temporary-directory and owner-marker device/inode identities, and deliberately leaks evidence when either identity is ambiguous. The post-publication receipt is explanatory and always records `training_authority=false`, `approval_inherited=false`, and `quarantine_inherited=false`.
