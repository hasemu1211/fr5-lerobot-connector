...
`preview-profile` accepts one exact `curator.up_view_profile_request.v2` JSON object. Paths may be absolute or relative to the request file. The review bundle and approval must have the same parent, and the approval must remain outside the immutable bundle directory.
...
```json
{
  ...
  "width": 640,
  "height": 480,
  "collection_camera_profile": "../../../config/data_factory/collection_profiles/fr5-up-wrist-rgb-30hz-v1.json",
...
The collection profile must be an exact `data_factory.collection_profile.v2` JSON object whose canonical digest matches the request. The curator mirrors the producer's v2 semantic checks for identifiers, camera roles/serials/topics, 30 Hz dimensions, QoS/queue/encoder/storage values, batch encoding, portability, and qualification; its up+wrist, 640×480 fields must also match the observable source contract. The resolved profile and receipt label this only `DECLARED_CONFIG_OBSERVABLE_MATCH`: current producer datasets do not embed an immutable collection-profile/device-placement lineage, and the profile itself uses runtime device binding. It must not be interpreted as proof that every source episode used one physical camera placement. That stronger claim requires future producer-owned lineage; the curator does not manufacture it.

The reference PNG must be an exact RGB export of `reference_frame_index` from the finalized source through the official LeRobot reader. One clean frame is a valid plate input; up to 31 sorted unique indices produce a deterministic per-pixel temporal median. The bound prevents an accidentally huge profile from decoding and stacking an unbounded number of RGB frames; the median uses in-place uint8 order statistics and preserves the previous even-count floor-average result without a full float64 median array. Because the repository has no authoritative camera-placement evidence yet, this first slice binds the approved review to the exact source dataset payload digest; a different source requires a new preview and approval. Moving the camera, changing the crop/resolution, geometry, margin, reference, or plate inputs also produces a new profile/review digest. A `VERIFIED` binding is accepted only from the producer-owned canonical `config/data_factory/region_bindings/` registry; external or test bindings may remain `PREPARED_NOT_VERIFIED` for draft preview but cannot issue production approval.
