...
def motion():
    ...
    for p in e.PHASES:
        ...
        if p=="GRIPPER_CLOSE":s["pause_after"]="GRASP_VERDICT"
    ...
    return {"schema_version":"fr5.motion_program.v2","robot_system_id":"fr5-lab-a","resolved_job_digest":"sha256:"+"a"*64,"binding_digests":digests,"frames":{"planning_frame":"base_link","planning_group":"fairino5_v6_group","tool_link":"wrist3_link"},"planning":{"pipeline_id":"pilz_industrial_motion_planner","ptp_planner_id":"PTP","lin_planner_id":"LIN","goal_tolerances":{"position_m":.001,"orientation_rad":.01,"joint_rad":.01},"max_joint_state_age_s":1},"execution_timeouts_s":{"heartbeat_lease":1,"cancel":1,"precontact_confirmation":30,"grasp_verdict":30,"semantic_verdict":30},"gripper_requirements":{"command_position_m":.01,"acceptable_feedback_m":{"min":.01,"max":.012},"velocity_percent":20,"force_percent":50,"evidence_digest":"sha256:"+"7"*64},"steps":phases}
...
class Test(unittest.TestCase):
 ...
 def test_fake_execution_holds_reset_and_faults(self):
  ...
  t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion()}));n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"settings-a"));t.velocity=80;bad=n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"settings-e"));self.assertEqual((bad["code"],bad["state"],t.started,s.blocked),("GRIPPER_SETTINGS_MISMATCH","APPROVED",[],[]))
  ...
  s.mark_blocked=mark;n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion()}));calls=[0]
