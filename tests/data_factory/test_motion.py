...
class Test(unittest.TestCase):
 ...
 def test_fake_execution_holds_reset_and_faults(self):
  class Live(T):
   ...
   def snapshot(self,*_):return {"joint_positions":self.position,"arm_controller":{"ready":True},"gripper_controller":{"ready":True}}
  ...
  def ready(verdict):
   clock=[0];t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),monotonic_clock=lambda:clock[0],cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));a={"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"};n.process(self.req("approve",a,"a"));n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e"));n.tick();n.tick();self.assertEqual(n.runs["r"]["state"],"PRECONTACT_HUMAN");n.process(self.req("confirm",{"run_id":"r","plan_digest":p["plan_digest"],"confirmed_by":"operator-1","source":"HUMAN"},"c"));n.tick();n.tick();n.tick();self.assertEqual(n.runs["r"]["state"],"SEMANTIC_VERDICT");n.process(self.req("semantic_verdict",{"run_id":"r","plan_digest":p["plan_digest"],"verdict":verdict,"decided_by":"operator-1","source":"HUMAN"},"v"));[n.tick() for _ in range(4)];return n,t,s,p
  ...
  t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"a2"));n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e2"));t.cancel_active=lambda *_:(_ for _ in ()).throw(e.ContractError("ROS_EXEC_CANCEL_ACK_TIMEOUT"));n.process(self.req("heartbeat",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1","recorder_health":{"writer_alive":False,"writer_error":None}},"h"));self.assertEqual((n.runs["r"]["failure_code"],n.runs["r"]["state"],bool(n.runs["r"]["execution"]["snapshot"]),n.runs["r"]["execution"]["cancel_error"]),("RECORDER_WRITER_FAULT","BLOCKED",True,"ROS_EXEC_CANCEL_ACK_TIMEOUT"));self.assertEqual(n.process(self.req("heartbeat",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"bad","recorder_health":{"writer_alive":True,"writer_error":None}},"badlease"))["code"],"LEASE_BINDING")
  clock=[0];t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),monotonic_clock=lambda:clock[0],cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"a4"));n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e4"));clock[0]=2;n.tick();self.assertEqual((n.runs["r"]["failure_code"],t.cancelled,t.started),("HEARTBEAT_TIMEOUT",1,["PREGRASP_PTP"]))
  t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"a5"));n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e5"));out=io.StringIO();self.assertFalse(e.run_jsonl(io.StringIO(),out,n));self.assertEqual((len(out.getvalue().splitlines()),__import__("json").loads(out.getvalue())["state"],t.started),(1,"BLOCKED",["PREGRASP_PTP"]))
  class Quiet:
   def __iter__(self):time.sleep(.2);return iter(())
  clock=[0];t=Live();s=Store();n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),monotonic_clock=lambda:clock[0],cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"a6"));n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e6"));clock[0]=2;out=io.StringIO();self.assertFalse(e.run_jsonl(Quiet(),out,n));self.assertEqual((len(out.getvalue().splitlines()),__import__("json").loads(out.getvalue())["code"],t.cancelled,t.started),(1,"HEARTBEAT_TIMEOUT",1,["PREGRASP_PTP"]))
  t=Live();s=Store();writes=[]
  def mark(*args):
   writes.append(args)
   if len(writes)>1:raise e.ContractError("STATE_WRITE_FAILED")
  s.mark_blocked=mark;t.snapshot=lambda *_:(_ for _ in ()).throw(e.ContractError("ROS_JOINT_STATE_STALE"));n=e.PickupExecutor(t,clock=lambda:datetime(2026,1,1,tzinfo=timezone.utc),cell_state_store=s,execution_enabled=True);p=n.process(self.req("plan",{"run_id":"r","motion_program":motion(),"initial_joint_state":[0]*6}));self.assertEqual(n.process(self.req("plan",{"run_id":"r2","motion_program":motion(),"initial_joint_state":[0]*6},"one"))["code"],"ONE_JOB_ONLY");n.process(self.req("approve",{"approval_id":"approval-1","approved_by":"operator-1","run_id":"r","resolved_job_digest":"sha256:"+"a"*64,"plan_digest":p["plan_digest"],"approval_expiry":"2026-01-02T00:00:00Z"},"a3"));bad=n.process(self.req("execute",{"run_id":"r","plan_digest":p["plan_digest"],"lease_id":"lease-1"},"e3"));self.assertEqual((bad["code"],bad["state"],bad["data"]["cell_state_error"],t.started),("ROS_JOINT_STATE_STALE","BLOCKED","STATE_WRITE_FAILED",[]))
