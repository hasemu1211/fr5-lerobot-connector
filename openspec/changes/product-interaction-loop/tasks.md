...
## 4. Stored collection advice becomes an actual draft

- [x] Preserve native pins and exclusions while proposing an unobserved qualified condition within the existing count bound; consume the native projector/compiler in the actual Collection application.
...
- [x] Discover and select existing immutable requests after process/browser restart without a prior local descriptor; bound metadata-only listing, distinguish discovery from native current eligibility, reject changed/unusable files, and verify zero replay plus unchanged source/request bytes through native and shipped UI consumers.
