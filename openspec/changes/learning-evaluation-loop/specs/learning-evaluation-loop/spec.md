...
- **AND** already-started EVAL pools are released after each evaluation before checkpoint serialization, while TRAIN remains persistent; the native worker-options seam logs this process-lifetime adaptation without rewriting the parent recipe or changing the consumed data/policy RNG
