...
수집·학습을 막는 결함 수정에 필요한 작은 책임 정리는 함께 수행한다. 루트부터 하위 깊이까지의 구조 검토는 첫 학습·평가 근거를 늦추지 않는 후속 개선으로 둔다. 실제 탐색 혼선·중복·반복 수정 비용이 확인된 경계부터 정리하며, 전면적인 폴더 재편을 선행 조건으로 삼지 않는다.
