...
- [ ] Rollout 기반 데이터 요구의 순환 검증: canonical 실패 evidence에서 runtime 교정과 데이터 요구를 구분하고, 선택한 수집 가설을 제품의 수집·선별·승인된 학습·재평가 경로로 연결한다. 같은 비용의 비교 기준, 고정 평가 조건과 불확실성을 보존하며 단순 추천 출력·synthetic 연결·단일 성공 주행으로 정책 개선을 주장하지 않는다.
