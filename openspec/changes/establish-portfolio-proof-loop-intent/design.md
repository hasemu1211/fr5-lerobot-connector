...
9. Portfolio의 기술 설명은 설계 의도 조사에서 시작한다. main MEX는 현재 맥락과 원본으로 가는 경로이며, 누락되거나 오래된 요약일 수 있다. 표현 책임자는 관련 사용자 의도·설계/변경 기록에서 왜 이 방법을 택했는지 찾고, 실제 호출 코드·입력 계약·테스트·산출물과 대조한다. 다른 지원 입력과 경계 조건에서 설명의 범위를 확인하고 필요한 외부 일차 자료로 원리를 검토한다. 확인한 설계 목적, 실제 메커니즘과 측정 효과를 구분한 뒤 목적이 먼저 읽히는 글·그림을 만든다. 발견한 source 결함은 재현 근거와 함께 기존 owner에게 전달하며, 표현의 문제는 이 lane에서 직접 고친다. 조사 기록이나 오독 방지 문구를 늘리는 것이 완료가 아니며, 새 근거나 의미 변경이 없는 문장 수정마다 같은 조사를 반복하지 않는다.

### 속도 정책의 시험과 적용

속도 후보는 임의의 scaling 비율이나 다른 모델의 추천을 합의된 기준으로 취급하지 않고, 실제 학습 시연의 작업 리듬과 FR5의 거리·회전량·구간별 실행 기록을 대조하여 정한다. 판단 대상은 이 데이터 생성 시스템에 적절한 시연 동작의 시간이며, scaling 비율은 그 움직임을 실행하기 위한 변환값이다. 비율별 예상 시간 계산은 병목 진단이지 해당 시간이 적절하다는 근거가 아니다. 비교에서는 시작 대기·그리퍼 대기·복귀와 실제 이동을 구분하고, 다른 로봇의 관절 값은 단위와 보정 근거 없이 각속도로 해석하지 않는다. 작은 팔의 시연 시간이나 비율을 FR5에 그대로 복사하지 않으며, 큰 팔이라는 이유만으로 전 구간을 일괄 감속하거나 접촉 직전이라는 명칭만으로 별도 저속을 요구하지도 않는다. 요청 속도·가속도는 실제 소비하는 planner와 장치 한계에 대응시킨다. 시연 비교는 후보를 정할 근거이며, 특정 속도의 실물 적격이나 학습 성능을 대신 증명하지 않는다.
...
- [조사가 실행을 대체하거나 자의적 선택의 장식이 될 수 있음] → 중요한 알고리즘·운용·아키텍처·배포 판단은 실제 데이터와 병목을 먼저 확인하고, 최신 논문·공식 구현·공개된 실제 운용 사례에서 대안을 비교한 뒤 현재 PC·FR5·source/tests에 맞는 부분을 선택한다. 근거가 직접 뒷받침하는 사실, 환경 차이에 대한 추론, 직접 검증할 공학적 가설을 구분한다. 출처가 제안하지 않은 수치나 보장을 인용으로 정당화하지 않는다. 채택 이유와 적용되지 않는 전제, 시간·품질·복구 비용으로 판단할 유지·수정 조건을 기존 owner의 evidence에 연결한다. 기존 프로젝트 리서치는 재사용하되 새 외부 근거와 실제 결과에 따라 갱신한다. 이미 검토된 통합이나 작은 기계적 수정을 위해 같은 조사를 반복하지 않는다.
