...
2. **목적이 먼저 읽히는 구성.** 짧은 소개와 적합한 시각 표현으로 무엇을 위한 기술인지 먼저 드러낸다. 원리·선택 이유·이점은 그 이해를 돕도록 배치하고 상세 계산과 근거는 필요할 때 탐색하게 한다. 자연스러운 도메인 용어를 일관되게 사용하며 공통 세부 템플릿을 강제하지 않는다.
...
4. **반복보다 설명 역할.** 각 구간은 독자가 새로 이해할 내용을 가진다. 같은 자료를 다시 쓸 때는 전체 동작과 정책 입출력처럼 서로 다른 관계를 보여 준다. 역할이 겹치면 합치거나 줄이며, 같은 설명을 본문·그림·접힌 상세에 중복해서 쌓지 않는다. 제목·그림·범례·버튼·링크가 이미 전달하는 내용을 문장으로 되풀이하고, 삭제해도 목적·원리·조건·결과 해석에 필요한 정보가 사라지지 않으면 그 문장을 삭제한다.
