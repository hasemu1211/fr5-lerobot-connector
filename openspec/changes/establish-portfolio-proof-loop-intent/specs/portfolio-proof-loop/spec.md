...
4. **반복보다 설명 역할.** 각 구간은 독자가 새로 이해할 내용을 가진다. 같은 자료를 다시 쓸 때는 전체 동작과 정책 입출력처럼 서로 다른 관계를 보여 준다. 역할이 겹치면 합치거나 줄이며, 같은 설명을 본문·그림·접힌 상세에 중복해서 쌓지 않는다.
...
- **AND** 단순 삭제나 주의 문구 추가로 끝내지 않고, 필요한 의미가 더 쉽게 읽히는지 확인한다
