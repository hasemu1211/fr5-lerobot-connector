...
2. **목적이 먼저 읽히는 구성.** 짧은 소개와 적합한 시각 표현으로 무엇을 위한 기술인지 먼저 드러낸다. 개요의 계층·개발 범위·데이터 흐름은 관계를 보여 주는 그림으로 우선 전달하며, 설명 문단을 도형 안에 옮겨 담는 것으로 시각화를 대신하지 않는다. 원리·선택 이유·이점은 그 이해를 돕도록 배치하고 상세 계산과 근거는 필요할 때 탐색하게 한다. 자연스러운 도메인 용어를 일관되게 사용하며 공통 세부 템플릿을 강제하지 않는다.
3. **차이가 보이는 시각화.** 물체·형상·조건은 원리와 인과관계가 잘 드러나도록 선택한다. 계산 결과를 제시하기 전에 독자가 그 결과가 생기는 이유를 따라갈 수 있게 한다. 비교에서는 설명할 변화와 고정할 조건을 구분하고, 서로 다른 개념에 비슷한 모양을 써서 혼동을 만드는지 검토한다. 설명용 형상과 강조는 실제 조건과 구분하며 수치·효과·구현 범위를 과장하지 않는다. 인터랙션은 조작 결과가 의미 있게 읽힐 때 사용하고, 정적인 비교가 더 명료하면 대체한다.
4. **반복보다 설명 역할.** 각 구간은 독자가 새로 이해할 내용을 가진다. 같은 자료를 다시 쓸 때는 전체 동작과 정책 입출력처럼 서로 다른 관계를 보여 준다. 역할이 겹치면 합치거나 줄이며, 같은 설명을 본문·그림·접힌 상세에 중복해서 쌓지 않는다. 영상·제목·그림·범례·버튼·링크가 이미 전달하는 내용을 문장으로 되풀이하고, 삭제해도 목적·원리·조건·결과 해석에 필요한 정보가 사라지지 않으면 그 문장을 삭제한다. 영상에서 명백한 동작을 문장으로 다시 서술하지 않으며, 삭제로 충분한 내용을 새 시각물로 중복 제작하지 않는다. 단위·비교 조건·측정 결과와 목표·구현의 구분처럼 해석에 필요한 정보는 보존한다.
5. **실제 독자 경험으로 개선.** 중요한 표현 선택은 서로 다른 우수 사례의 구성·설명 방식과 대조하고, 실제 화면에서 내용 위계·정렬·가독성·비교 효과·탐색 흐름을 직접 검토한다. 자동 검사 통과를 시각적 완성도로 간주하지 않는다. 발견된 결함은 가장 작은 효과적인 수정으로 해결하며 매 편집의 승인, 반복 조사, 전면 재디자인을 기본 절차로 만들지 않는다.
