...
데이터가 늘어날 때 기존 평가 identity는 Learning의 planning-only cohort로
동결하고 native mapping/request/CLI가 그 원본 provenance를 재검증해 소비한다.
새 원본은 TRAIN으로 들어가며, 평가 fraction이나 source 순서를 조정해 고정 평가를
흉내 내지 않는다. 준비 성공은 새 batch의 training authority나 학습 효용이 아니다.
