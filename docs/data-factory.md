...
[collection_recommendation_io](../tools/data_factory/collection_recommendation_io.py)는 명시적으로 실행하는 offline CLI/library 소비자다. `python3 -m tools.data_factory.collection_recommendation_io --help`가 옵션의 정본이며, 동일 campaign의 run directories와 분석 구현의 source commit을 받는다. source commit은 과거 수집 코드의 추정값이 아니다. 호출자가 claims, verdicts, patches를 조립하지 않아도 ledger/state/candidate와 참조 artifact를 정본 validator로 확인하고, 기존 [coverage owner](../tools/data_factory/quality/coverage_report.py)의 report를 만들어 advisory recommendation으로 연결한다. 이 분석은 live collection의 선행 조건이나 background daemon이 아니다.

측정 범위는 보존된 qualified domain과 입력으로 제공한 episode 집합이다. 과거 aggregate counts는 중복 여부를 알 수 없어 합산하지 않는다. coverage owner가 아직 관측되지 않은 qualified condition을 제안할 때만 해당 condition 수에 한 번씩 대응하는 유한한 requested-count 변경을 제안하며, 기존 UI 상한을 지킨다. 이미 모두 관측한 domain에는 collect-more를 제안하지 않는다. 이 count 제안은 특정 pose 선택, 전체 이력의 데이터 부족, 충분한 품질이나 정책 효과를 입증하지 않는다. 실제 셀 선택과 새로운 편집·compile·승인은 기존 owner의 별도 책임이다.

출력은 source roots 밖의 전용 derived root 아래 recommendation digest별 디렉터리에 canonical coverage report와 recommendation을 보존한다. 같은 입력은 같은 결과를 재사용하고, 바뀐 episode/state/report/분석 commit은 다른 결과를 만든다. 기존 결과가 변조되거나 불완전하면 덮어쓰지 않고 실패한다. 전체 compiled authoring이 없는 legacy run, 서로 다른 campaign, digest 불일치, 누락된 candidate는 typed `UNAVAILABLE`이며 현재 config로 복원하지 않는다. 현재 recommendation 계약은 v2 campaign manifest와 중복 없는 manifest-order prefix episode 입력을 요구한다.

선택한 제안은 [project_update_draft_intent](../tools/data_factory/collection_recommendation.py)를 통해 현재 operator view에 묶인 intent로 만들고 기존 `update_draft` owner가 적용한다. stale view는 거부된다. 추천 자체는 compile, authorize, recorder, motion, training을 실행하지 않는다. vision/person/background/robot variation과 physical rollout은 `UNKNOWN`으로 남고, semantic review가 pending이면 semantic proof도 `UNKNOWN`이다. 합성 테스트는 저장·재소비·replay·변경 입력·draft round-trip을 증명하며, 실물 수집·rollout 효과나 자동 후속 실행을 증명하지 않는다.
