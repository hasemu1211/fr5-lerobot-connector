...
![처리 모듈과 전달 산출물을 구분한 상세 구조. 원본 조건·판정에서 학습 요청과 다음 수집 추천으로 이어지고, FR5 학습 연결 코드와 외부 학습기를 구분한다. 기존 유한 실행 진단 소비와 연속 실행의 후속 연결을 별도로 표시한다.](portfolio/diagrams/system-architecture.svg)

원본 데이터와 수집 조건·판정 기록은 함께 추적하지만 같은 파일은 아니다. Selection은 이를 참조한 학습 요청을 만들며, 학습 승인은 별도 단계에서 확인한다. FR5의 학습 연결 코드가 분할과 TRAIN 통계를 적용한 뒤 LeRobot 학습기를 호출한다. 실행 진단은 기존 유한 실행의 소비 경로와 연속 실행의 후속 연결을 구분했다.
