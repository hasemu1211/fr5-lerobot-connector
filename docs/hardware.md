...
- retune은 PHYSICAL `TEST_ONLY` 조립에서만 `wood-cube-25mm-r001` + `wood-cube-25mm-top-center-r001`에 적용된다. production/training authority는 없고 상태는 `CANDIDATE_PENDING_HIL`이다. 성공하면 새 grasp profile revision 승격을 검토하고, 실패하면 이 조정 파일만 바꾼다. 다른 물체와 기본 resolver는 각자의 qualified profile을 계속 사용한다.
