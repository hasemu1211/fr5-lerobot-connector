...
browser는 robot, recorder, dataset, campaign 또는 review state의 owner가 아니다. server가 제공한 token은 local page channel의 possession만 증명하고 OS 인증이나 사람의 신원을 증명하지 않는다. stale view, replayed intent, revision rollback, unknown enum, cancel-pending과 bridge 장애에서는 mutation을 거부하고 마지막으로 수락한 projection만 표시한다.
