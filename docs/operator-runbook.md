...
물리 caller의 이동 전 준비 검사는 기존 짧은 녹화 구간에서 상단 카메라 밝기도 확인한다. 기준보다 어둡거나 밝기 증거가 없으면 새 motion을 보내지 않고 준비 녹화를 정리한다. 손목 카메라 가림과 일반 영상 품질 경고는 이 시작 조건으로 사용하지 않는다. 기준은 recorder readiness 계약이 소유하며, 이는 물체 인식·작업 성공 판정이나 안전용 조도 센서가 아니다. 현재 동작 중 즉시 소등 감지는 제공하지 않으며, 동작 중 조명이 꺼지면 다음 이동 전 검사에서 차단한다. 실제 카메라의 자동 노출을 포함한 소등 재현 검증 전에는 무인 운전의 보장으로 해석하지 않는다.
