...
if (observations) {
  ...
  async function render() {
    ...
    try { const url = new URL(location.href); url.searchParams.set('episode', episode); url.searchParams.set('moment', moment); history.replaceState(null, '', url.search + url.hash); }
    ...
    const source = new URL(document.getElementById('frame-source').getAttribute('href'), location.href);
    source.hash = `ep${episode}-f${item.frame}`;
    document.getElementById('frame-source').href = 'sources/observations.html' + source.search + source.hash;
    renderAction(item);
