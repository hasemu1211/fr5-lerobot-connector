...
Usage: python3 docs/portfolio/build/export_single_file.py /path/to/FR5-Portfolio.html
...
function show(url) {
  ...
  for (const el of doc.querySelectorAll('script[src]')) {
    ...
    if (path==='scripts/observations.js') code+='\nwindow.FR5_OBSERVATIONS.forEach(row=>{row.images=row.images.map(parent.fr5Asset)});';
    if (path==='scripts/learning.js' || path==='scripts/collection.js')
...
def export(output: Path) -> tuple[int, str]:
    ...
    for required in ('index.html', 'scripts/data.js', 'scripts/learning.js', 'scripts/connections.js', 'scripts/source-return.js'):
        assert required in files, required
    for name in ('scripts/learning.js', 'scripts/collection.js'):
        assert len(re.findall(r'const src = (`[^`]+`);', (root / name).read_text(encoding='utf-8'))) == 1, name
